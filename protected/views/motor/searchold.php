  ...
  if($_REQUEST['name'])
      {
        ...
        // echo $_REQUEST['name']."test";die;
        // $motormodls=  MotorModel::model()->findAll();
        
 $motormodls=MotorModel::model()->findAllByAttributes(array('make_id'=>$_REQUEST['name']));
...
<div class="col-md-6 col-sm-6 col-xs-12" style="margin-top:5px !important;">
          <label class=" " >
    <input type="checkBox" name="Search[motor_status1]" value="1"  <?php if(!empty($mot_1)){ echo "checked";} ?> >
   Used
          </label>   
    <label class=" " >
    <input type="checkBox" name="Search[motor_status2]" value="2" <?php if(!empty($mot_2)){ echo "checked";} ?>>
    Nearly New  
    </label>      
    <label class=" " >      
    <input type="checkBox" name="Search[motor_status3]" value="3" <?php if(!empty($mot_3)){ echo "checked";} ?>>
     New</label>  
    </div>
                    ...
                    <div href="<?= Yii::app()->request->baseUrl ?>/<?= $controller ?>/item?id=<?= $product->id ?>" class="col-md-4 col-sm-4 col-xs-12 car-item">
                        ...
                        <div class="col-md-11 col-xs-11 item-details">
                            ...
                            if (!empty(Yii::app()->user->id)) {
                                ...
                                if ($check == 1) {
                                    ?>
                     <a class="fav_star rate_solid" href="javascript:void(0);" onclick="removefav(<?php echo  $product->id;?>)"></a>
                                <?php } else { ?>
             <a class="fav_star rate" href="javascript:void(0);" onclick="getProdcutId(<?php echo  $product->id;?>)" ></a>
                                    <?php
                                }
                            } else {
                                ?>
...
<script>
  ...
  var make =document.getElementById("makee").selectedIndex;
    //window.location.href = "search?name=" + make;
 $.ajax({
            url:"<?=Yii::app()->request->baseUrl?>/motor/search?name="+make,
            success:function (data){
                return true;
            }
        });
