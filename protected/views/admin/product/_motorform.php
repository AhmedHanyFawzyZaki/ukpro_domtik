...
<div class="control-group">
            ...
            <div class="controls">
                ...
                <?php
                echo $form->dropDownList($model, 'product_category_id', CHtml::listData(ProductCategory::model()->findAll(array('condition'=>'category_id=5')), 'id', 'title'), array(
...
<p style="color:red">please upload Main Image  with 290 height , 360 width</p>
<div class='control-group'>
...
<div  id="manufacure">
    <div class="control-group">
        <label for="make_id" class="control-label">Manufacure</label>
        <div class="controls">
    ...
    <div style="display:block" id="dpt">
        <div class="control-group">
            <label class="control-label" style="color:red"> Models</label>
            <div class="controls"  id="model">
                <?php
                  if (empty($productdetails->make_id))
                    echo
                    'select Manufacure first';
                else {
                    echo $form->dropDownList($productdetails, 'motor_model_id', CHtml::listData(MotorModel::model()->findAll(array('condition' => "make_id=$productdetails->motor_model_id")), 'id', 'title'), array('prompt' => 'Select Motors Models'));
                }
                ?>
            </div>
        </div>
...
<p><?php  echo $form->dropDownListRow($productdetails, 'gas_id',CHtml::listData(Gas::model()->findAll(), 'id', 'title'),array('prompt'=>'select Gas','class'=>'listtxt'));?></p>

 
<p><?php  echo $form->dropDownListRow($productdetails, 'door_id',CHtml::listData(Door::model()->findAll(), 'id', 'title'),array('prompt'=>'select Door','class'=>'listtxt'));?></p>

 
<p><?php  echo $form->dropDownListRow($productdetails, 'kmage_id',CHtml::listData(Kmage::model()->findAll(), 'id', 'title'),array('prompt'=>'select Kmage','class'=>'listtxt'));?></p>

 
<p><?php  echo $form->dropDownListRow($productdetails, 'age_id',CHtml::listData(Age::model()->findAll(), 'id', 'title'),array('prompt'=>'select Age','class'=>'listtxt'));?></p>

 
<p><?php  echo $form->dropDownListRow($productdetails, 'emission_id',CHtml::listData(Emission::model()->findAll(), 'id', 'title'),array('prompt'=>'select Emission','class'=>'listtxt'));?></p>

 
<p><?php  echo $form->dropDownListRow($productdetails, 'engine_id',CHtml::listData(Engine::model()->findAll(), 'id', 'title'),array('prompt'=>'select Engine','class'=>'listtxt'));?></p>


 <p><?php echo $form->dropDownListRow($productdetails, 'power_engine', array("1" => "100 CV ","2" => "200CV",array('prompt'=>'select Motor Status'))); ?></p>

<?php echo $form->textAreaRow($productdetails, 'conditions', array('class' => 'span5', 'maxlength' => 255)); ?>


<p><?php echo $form->dropDownListRow($model, 'on_sale', array("0" => "Not on sale ","1" => "On sale"), array('id' => 'sale')); ?></p>
<?php echo $form->dropDownListRow($productdetails,'motor_status',array("1"=>"Used","2"=>"Newly Used","3"=>"New")); ?>
